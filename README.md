...
Hmm, I got to push the reset on my approach. Finding quadratic coefficients that factor over the factor base is very easy and fast. I wonder... hmm.... I'm overcomplicating things too much. Like if we have 148 **2 = 4356 mod 4387, and lets say 4356 factors over the factor base... in this case its happens to be the other square.. but what if it isnt the other square but still factors over the factor base? Isn't that enough? In this case 148 ** 2  - 4*n = 4356 mod 4387 ... so it does loop around N 4 times.. which is the only condition we need (loop around N atleast once).. so we can also create squares and just have them create small numbers mod N... perhaps such is a better approach.

I'll go for a run and think about this... because if we're just summing what I refered to as "partial results" of squared quadratic coefficients.. we just need to find sums that are small mod N.... which would seem like a much cleaner approach to creating small numbers mod N.
